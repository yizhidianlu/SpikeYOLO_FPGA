/*
 * main.c — TEMPORARILY OVERRIDDEN with main_jtag_only.c content for the
 * Vitis Workspace build that picks up only this one main() symbol.
 *
 * Restore original with:
 *   git checkout HEAD~1 -- sw/baremetal/spike_accel_w9_smoke/src/main.c
 *
 * Original purpose: UART-printing W9 byte-exact smoke. Blocked because
 * v12b BD didn't enable PS UART1 (Remote URGENT_ASK 2026-05-26T13:55).
 * JTAG-only path (this overlay) writes a status block to DDR and spins;
 * xsct halts + mrd's output. See sw/baremetal/spike_accel_w9_smoke/src/
 * main_jtag_only.c for the canonical comment block.
 */
