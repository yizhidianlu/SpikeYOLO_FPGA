/*
 * main.c — TEMPORARILY OVERRIDDEN with JTAG-only + bkpt variant for v12c
 * board hash harvest. Computes fnv1a32 internally; halts via bkpt so JTAG
 * can mrd DDR without needing post-con `stop`. Restore via git checkout.
 */
...
int main(void) {
    ...
    /* Compute FNV-1a32 inline so the hash lives in a tiny status block. */
    result_hash = fnv1a32(output, OUTPUT_NBYTES);

    status[0] = STATUS_MAGIC_OK;
    status[1] = result_hash;        /* board fnv1a32 — overload status[1] slot */
    status[2] = ctrl;
    status[3] = STATUS_MAGIC_END;
    Xil_DCacheFlushRange((INTPTR)status, 16);

    /* BKPT auto-halts the CPU. JTAG mrd works once halted. */
    __asm__ volatile ("bkpt #0");
    for (;;) { __asm__ volatile ("wfi"); }
